...
function generateOrgIdFromName() {
    ...
    while (organizationsData && organizationsData.organizations.some(org => org.id === finalId)) {
        counter++;
...
async function saveOrganization(event) {
    ...
    // Validate org ID format (more permissive regex)
    if (!/^org-[-a-z0-9]+$/.test(orgId)) {
        showAlert('Organization ID must follow format: org-yourname-001 (lowercase, hyphens allowed)', 'warning');
        return;
    }

    // Additional validation: ensure ID is not empty after org- prefix
    if (orgId === 'org-' || orgId.length < 5) {
        showAlert('Organization ID is too short. Please enter a valid organization name.', 'warning');
        return;
    }
