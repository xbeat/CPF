...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={Security Orchestrator with Psychological Intelligence},
...
% Title
{\LARGE \textbf{Security Orchestrator with Psychological Intelligence:}}\\[0.3cm]
...
\begin{minipage}{0.48\textwidth}
\centering
{\Large Kashyap Thimmaraju}\\[0.2cm]
...
Human factors cause 85\% of successful security breaches \cite{verizon2023}, yet organizational responses remain reactive and periodic. Extensive research documents how psychological factors drive security failures, including the effects of stress and well-being among SOC practitioners \cite{thimmaraju2025}. Quarterly security assessments capture psychological vulnerabilities through static snapshots that become obsolete within days as conditions shift \cite{parsons2014}. What's needed is not more frequent assessment but continuous intelligence---a system that monitors psychological state space as vigilantly as network traffic, detects emerging vulnerabilities before exploitation, and orchestrates appropriate responses \cite{zimmermann2019}.
...
A critical yet neglected aspect is detecting when legitimate users become vulnerable to external manipulation due to psychological states. This is a long-standing problem that has not been effectively resolved. Security awareness training programs, for instance, are largely ineffective according to industry practitioners \cite{bada2019}---they fail to recognize that psychological vulnerability is dynamic and context-dependent, not a static knowledge deficiency. With the introduction of agentic LLMs into organizations, this problem amplifies: the agents themselves are prone to psychological manipulation, either by malicious agents or by malicious humans. The attack surface expands from vulnerable humans to vulnerable human-agent systems, making continuous psychological state monitoring even more urgent.

Recent advances in Large Language Models (LLMs) enable a fundamentally different architecture: multi-agent systems where specialized AI agents collaborate to solve complex problems \cite{xi2023, wang2023agents}. We present such a system where an orchestrator continuously monitors a psychological state matrix, detects anomalies and convergence patterns, and dynamically activates specialist agents to investigate and respond. This approach shifts from ``detect-and-alert'' to ``understand-and-act,'' creating what we term a \textit{Security Orchestrator with Psychological Intelligence}.
...
\item An adaptive learning module that continuously refines detection thresholds, discovers new vulnerability patterns from incident feedback, and self-optimizes without manual tuning
...
\end{equation}
where $X_i(t)$ is the observed value of indicator $i$ at time $t$, $\alpha = e^{-\Delta t/\tau_i}$ is the decay factor, and $\tau_i$ is the psychological half-life of indicator $i$. The max operation ensures acute observations immediately elevate state.
...
Our hybrid approach leverages deterministic rules for routine operations while reserving agent intelligence for complex decisions requiring contextual understanding. This reduces LLM API costs by approximately 85\% while maintaining detection quality (accuracy and completeness).
...
Where:
\begin{itemize}[leftmargin=*]
\item $C_k(t)$ = category $k$ score at current time $t$
\item $C_k(t-\Delta t)$ = category $k$ score one time interval $\Delta t$ in the past
\item $C_k(t-2\Delta t)$ = category $k$ score two time intervals in the past
\item $\Delta t$ = time interval (typically 1 day for continuous monitoring)
\item $a_k$ = acceleration (second derivative) of category $k$ score
\end{itemize}
...
|    * Makes decisions using current Theta, W, P  |
...
|    * Generates updated Theta', W', P'           |
+----------------+--------------------------------+
                 |
                 v
+-------------------------------------------------+
| 3. VALIDATION PHASE                             |
|    * Backtest Theta', W', P' on held-out data   |
...
\subsection{Security Orchestrator Paradigm}

This system represents a new operational paradigm: not passive monitoring nor fully automated response, but \textit{active intelligence partnership}. The orchestrator functions as a security analyst's orchestrator, continuously monitoring psychological state space, investigating anomalies, and recommending actions. Human analysts remain in control but are augmented by AI that understands human vulnerabilities \cite{jeong2019, workman2008}.
...
The author thanks the CPF research community for foundational theoretical work, and early adopters providing feedback on prototype deployments.

\section*{Code and Data Availability}

Reference implementation, agent prompt templates, and anonymized datasets: \url{https://cpf3.org/orchestrator}
...
Thimmaraju, K., Rispens, S. I., \& Ahn, G. (2025). Human performance in security operations: A survey on burnout, well-being and flow state among practitioners. In \textit{Proc. 2025 Workshop on Security Operations Center Operations and Construction (WOSOC 2025)}.
