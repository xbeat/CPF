...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={Security Orchestrator con Intelligenza Psicologica},
...
% Title
{\LARGE \textbf{Security Orchestrator con Intelligenza Psicologica:}}\\[0.3cm]
...
I fattori umani causano l'85\% delle violazioni di sicurezza di successo \cite{verizon2023}, eppure le risposte organizzative rimangono reattive e periodiche. Ampie ricerche documentano come i fattori psicologici guidino i fallimenti di sicurezza, inclusi gli effetti dello stress e del benessere tra i professionisti SOC \cite{thimmaraju2025}. Le valutazioni di sicurezza trimestrali catturano le vulnerabilità psicologiche attraverso snapshot statici che diventano obsoleti nel giro di giorni mentre le condizioni cambiano \cite{parsons2014}. Ciò che serve non è una valutazione più frequente ma un'intelligenza continua---un sistema che monitora lo spazio dello stato psicologico con la stessa vigilanza del traffico di rete, rileva vulnerabilità emergenti prima dello sfruttamento e orchestra risposte appropriate \cite{zimmermann2019}.
...
Un aspetto critico ma trascurato è il rilevamento di quando utenti legittimi diventano vulnerabili alla manipolazione esterna a causa di stati psicologici. Questo è un problema di lunga data che non è stato risolto efficacemente. I programmi di security awareness training, per esempio, sono ampiamente inefficaci secondo i professionisti del settore \cite{bada2019}—falliscono nel riconoscere che la vulnerabilità psicologica è dinamica e dipendente dal contesto, non una carenza di conoscenza statica. Con l'introduzione di agenti LLM nelle organizzazioni, questo problema si amplifica: gli agenti stessi sono suscettibili alla manipolazione psicologica, sia da parte di agenti malevoli che di umani malevoli. La superficie di attacco si espande da umani vulnerabili a sistemi umano-agente vulnerabili, rendendo ancora più urgente il monitoraggio continuo dello stato psicologico.
...
Questo lavoro introduce il \textbf{Security Orchestrator}: un'architettura multi-agent che trasforma il CPF da framework di valutazione a sistema di intelligence operativa. A differenza dei sistemi tradizionali che trattano gli esseri umani come "il weak link" da gestire \cite{sasse2001}, il nostro approccio riconosce che comprendere la vulnerabilità psicologica richiede un ragionamento continuo e sofisticato---un compito per il quale gli LLM agent sono particolarmente adatti.

\subsection{Contributi}

Questo lavoro apporta i seguenti contributi principali:

\textbf{1. Security Orchestrator come Paradigma}. Introduciamo il concetto di Security Orchestrator: un sistema AI che comprende le vulnerabilità psicologiche umane con la stessa profondità delle superfici di attacco tecniche. Proprio come i code co-pilot assistono gli sviluppatori comprendendo l'intento del codice, i security orchestrator assistono i team di sicurezza comprendendo lo stato psicologico umano. Questo rappresenta un cambio fondamentale dal trattare gli esseri umani come problemi di sicurezza al comprenderli come sistemi complessi che richiedono monitoraggio e supporto intelligenti.
...
\item Modulo di adaptive learning che raffina continuamente le soglie di detection, scopre nuovi pattern di vulnerabilità dal feedback incidenti e si auto-ottimizza senza tuning manuale
\end{itemize}
...
\section{Il Security Orchestrator: Concetto e Design}
...
Il paradigma Security Orchestrator affronta queste limitazioni attraverso il monitoraggio continuo automatizzato abbinato al ragionamento esperto. Proprio come un code co-pilot assiste gli sviluppatori comprendendo l'intento del codice e suggerendo completamenti, un security orchestrator assiste i team di sicurezza comprendendo lo stato psicologico e identificando vulnerabilità emergenti.
...
\begin{verbatim}
    ...
    └──────────┘        └──────────┘        └──────────┘
         └─────────────────┬──────────────────┘
                     ...
                     └─────────────┘
\end{verbatim}
\caption{Architettura Security Orchestrator a tre livelli}
\end{figure}
...
Questo design ibrido riduce i costi LLM dell'85\% pur mantenendo la qualità del rilevamento (accuracy e completeness). La maggior parte degli aggiornamenti (incrementi singoli, decadimento) costano millisecondi di tempo CPU. Solo ~15\% degli eventi attivano l'analisi LLM.
...
\begin{verbatim}
Sei il Security Orchestrator. Il tuo ruolo è mantenere 
...
Dove:
\begin{itemize}[leftmargin=*]
\item $C_k(t)$ = punteggio della categoria $k$ al tempo attuale $t$
\item $C_k(t-\Delta t)$ = punteggio della categoria $k$ un intervallo di tempo $\Delta t$ nel passato
\item $C_k(t-2\Delta t)$ = punteggio della categoria $k$ due intervalli di tempo nel passato
\item $\Delta t$ = intervallo di tempo (tipicamente 1 giorno per il monitoraggio continuo)
\item $a_k$ = accelerazione (seconda derivata) del punteggio della categoria $k$
\end{itemize}
...
Questo lavoro introduce il paradigma Security Orchestrator: sistemi AI che comprendono le vulnerabilità psicologiche umane con la stessa profondità delle superfici di attacco tecniche. I contributi chiave includono:
...
\textbf{3. Filtraggio Ibrido per Efficienza dei Costi}. Gestione deterministica degli aggiornamenti di routine, analisi LLM per pattern complessi---riduzione dei costi dell'85\% pur mantenendo qualità del rilevamento (accuracy e completeness).
...
Abbiamo introdotto il paradigma Security Orchestrator: sistemi AI che comprendono le vulnerabilità psicologiche umane con la stessa profondità delle superfici di attacco tecniche. Proprio come i code co-pilot assistono gli sviluppatori comprendendo l'intento del codice, i security orchestrator assistono i team di sicurezza comprendendo lo stato psicologico umano.

Il nostro sistema multi-agent raggiunge questo attraverso tre contributi chiave:

\textbf{1. Architettura con Intelligenza Psicologica}. Un orchestratore mantiene una matrice di stato psicologico 100-dimensionale per ogni utente, rilevando vulnerabilità convergenti e attivando dinamicamente agent specialisti per investigazione profonda. Il filtraggio ibrido gestisce gli aggiornamenti di routine in modo deterministico, riservando analisi LLM per pattern complessi---riduzione dei costi dell'85\% pur mantenendo qualità del rilevamento (accuracy e completeness).
